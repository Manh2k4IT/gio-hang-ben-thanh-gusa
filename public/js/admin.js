...
async function load() {
  ...
  try {
    ...
    if (headerTitle) {
      headerTitle.textContent = category ? `📦 Quản lí đơn hàng kho vải bến thành - ${category}` : "📦 Quản lí đơn hàng kho vải bến thành";
    }
    if (headerDesc) {
      headerDesc.textContent = category ? `Đang quản lý riêng danh mục ${category}` : "Quản lý sản phẩm kho vải bến thành";
    }
    if (addBtn) {
