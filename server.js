...
function normalizeProductOrder() {
    ...
    products.forEach((product, index) => {
        ...
        const variantRowCount = getProductVariantRowCount({
            ...product,
            images: normalizedImages,
            variantNames: rawNames
        }, rawNames.length || normalizedImages.length || 1);
        product.variantNames = Array.from({ length: variantRowCount }, (_, nameIndex) => rawNames[nameIndex] || `Màu ${nameIndex + 1}`);
        product.sizes = normalizeSizes(product.sizes);
        const normalizedVariantSizes = normalizeVariantSizes(product.variantSizes, variantRowCount);
...
function normalizeOptionalOldPrice(value, fallback = null) {
...
function getProductVariantRowCount(product, fallback = 1) {

    const images = normalizeImageList(product?.images || product?.image);
    const names = normalizeVariantNames(product?.variantNames);
    const variantColorStocks = Array.isArray(product?.variantColorStocks) ? product.variantColorStocks : [];
...
function normalizeVariantStocks(value, variantCount = 0) {
    ...
    return Array.from({ length: normalizedLength }, (_, index) => {
        ...
        if (Number.isFinite(Number(row))) {
            return normalizeStockValue(row, 0);
        }
        ...
        if (row && typeof row === "object") {
            const total = Object.values(row).reduce((sum, qty) => {
                ...
                return sum + normalizeStockValue(n, 0);
            }, 0);
...
function normalizeVariantPrices(value, variantCount = 0) {
...
function getVariantUnitCutLength(product, variantIndex) {

    const index = Math.max(0, Number(variantIndex) || 0);
    const direct = Array.isArray(product?.variantCutLengths)
        ? Number(product.variantCutLengths[index])
        : NaN;
    ...
    const parsed = extractCutLengthFromVariantName(variantNames[index]);
...
function normalizeVariantCutLengths(value, variantCount = 0) {
...
function extractCutLengthFromVariantName(name) {
...
function getVariantUnitPrice(product, variantIndex) {

    const index = Math.max(0, Number(variantIndex) || 0);
    const fallback = normalizeNumberValue(product?.price, 0);
    const variantCount = getProductVariantRowCount(product, 1);
    const prices = normalizeVariantPrices(product?.variantPrices, variantCount);
...
function normalizeCategoryList(values) {
    ...
    if (!ordered.some((item) => normalizeCategoryName(item) === normalizeCategoryName("Khác"))) {
        ordered.push("Khác");
    }

    return ordered.length ? ordered : cloneData(DEFAULT_SETTINGS.productCategories);
...
app.put("/settings/categories", (req, res) => {

    const category = normalizeCategoryLabel(req.body?.category);
    if (!category) {
        return res.status(400).json({ error: "Thiếu tên danh mục" });
    }

    if (category.length > 80) {
        return res.status(400).json({ error: "Tên danh mục quá dài" });
    }

    const current = getCategoryOptions();
    appSettings.productCategories = normalizeCategoryList([...current, category]);
    updateClient();

    res.json({
        success: true,
        productCategories: appSettings.productCategories
    });
...
app.post("/product/add", (req, res) => {
    ...
    const normalizedVariantNames = Array.from({ length: variantRowCount }, (_, index) => rawVariantNames[index] || `Màu ${index + 1}`);
    const normalizedSizes = normalizeSizes(sizes);
    const normalizedVariantSizes = normalizeVariantSizes(variantSizes, variantRowCount).map((row) => {
        const normalizedRow = normalizeSizes(row);
...
app.put("/product/:id", (req, res) => {
    ...
    if (variantNames !== undefined || images !== undefined || image !== undefined) {
        ...
        product.variantNames = Array.from({ length: variantRowCount }, (_, index) => rawNames[index] || `Màu ${index + 1}`);
        product.variantSizes = normalizeVariantSizes(product.variantSizes, variantRowCount).map((row) => {
            const normalizedRow = normalizeSizes(row);
    ...
    if (variantSizes !== undefined) {
        ...
        product.variantSizes = normalizeVariantSizes(variantSizes, variantRowCount).map((row) => {
            const normalizedRow = normalizeSizes(row);
            return normalizedRow.length ? normalizedRow : normalizeSizes(product.sizes);
        });
    }
    if (variantStocks !== undefined) {
        const variantRowCount = getProductVariantRowCount(product, Array.isArray(variantStocks) ? variantStocks.length : 1);
        product.variantStocks = normalizeVariantStocks(variantStocks, variantRowCount);
        product.variantColorStocks = [...product.variantStocks];
    }
    if (variantColorStocks !== undefined) {
        const variantRowCount = getProductVariantRowCount(product, Array.isArray(variantColorStocks) ? variantColorStocks.length : 1);
        product.variantColorStocks = normalizeVariantStocks(variantColorStocks, variantRowCount);
        product.variantStocks = [...product.variantColorStocks];
    }
...
server.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
        ...
        console.error(`Cổng ${currentPort} đang bị chiếm. Hãy tắt tiến trình cũ trước khi chạy lại server.`);
        process.exit(1);
    } else {
        console.error(err);
        process.exit(1);
